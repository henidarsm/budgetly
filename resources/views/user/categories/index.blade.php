...
@section('page-subtitle', 'Kelola kategori untuk pengelompokan pengeluaran')
...
<div class="row g-4">
            ...
            <div class="p-4">
                ...
                <form method="POST" action="{{ route('user.categories.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="cth: Bahan Baku, Transport..." value="{{ old('name') }}" required>
                    </div>
                    <div class="mb-3">
    ...
    <div class="col-lg-8">
        ...
        <div class="card-custom mb-4">
            <div class="card-header-custom">
                <h6><i class="fas fa-user-tag me-2" style="color:#1565C0;"></i>Kategori Saya ({{ $myCategories->count() }})</h6>
            </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="p-3 rounded-3 d-flex align-items-center gap-3" style="background:#F8FAFF;border:1px solid #E8F0FE;">
                            <div style="width:42px;height:42px;background:{{ $cat->color }}20;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;border:2px solid {{ $cat->color }}40;">
                                <i class="fas fa-tag" style="color:{{ $cat->color }};font-size:16px;"></i>
                            </div>
                            <div class="flex-grow-1">
                                <div style="font-weight:700;font-size:14px;color:#1E293B;">{{ $cat->name }}</div>
                                <div style="font-size:12px;color:#94A3B8;">{{ $cat->expenses_count }} pengeluaran</div>
                            </div>
                            <div class="d-flex gap-1">
                                <button class="btn-icon btn-blue" onclick="openEditModal({{ $cat->id }}, '{{ $cat->name }}', '{{ $cat->color }}', '{{ $cat->description }}')" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </button>
                                @if($cat->expenses_count == 0)
                                <form method="POST" action="{{ route('user.categories.destroy', $cat) }}">
                                    @csrf @method('DELETE')
        ...
        <div class="card-custom">
            <div class="card-header-custom">
                <h6><i class="fas fa-globe me-2" style="color:#7C3AED;"></i>Kategori Global (dari Admin) ({{ $globalCategories->count() }})</h6>
            </div>
            <div class="p-3">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="p-3 rounded-3 d-flex align-items-center gap-3" style="background:#FAFBFF;border:1px solid #E8F0FE;">
                            ...
                            <div class="flex-grow-1">
                                <div style="font-weight:700;font-size:14px;color:#1E293B;">{{ $cat->name }}</div>
                                <div style="font-size:12px;color:#94A3B8;">{{ $cat->expenses_count }} pengeluaran &bull; Global</div>
                            </div>
