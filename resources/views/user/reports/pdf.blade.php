...
<head>
    <meta charset="UTF-8">
    <title>Laporan Pengeluaran - {{ $user->name }}</title>
    <style>
...
<div style="padding: 12px 30px; background: #DBEAFE; display: table; width: 100%;">
    <div style="display: table-cell; width: 50%;">
        ...
        <div style="font-size: 13px; font-weight: bold; color: #1E293B;">{{ $user->name }}</div>
    </div>
    <div style="display: table-cell; width: 50%;">
        <div style="font-size: 10px; color: #1E40AF; text-transform: uppercase; font-weight: bold;">Nama Usaha</div>
        <div style="font-size: 13px; font-weight: bold; color: #1E293B;">{{ $user->business_name ?? '-' }}</div>
    </div>
</div>
<div style="padding: 10px 30px; background: #EFF6FF; display: table; width: 100%;">
    <div style="display: table-cell; width: 33%;">
        ...
        <div style="font-weight: bold;">{{ \Carbon\Carbon::parse($dateFrom)->format('d F Y') }}</div>
    </div>
    <div style="display: table-cell; width: 33%;">
        <div style="font-size: 10px; color: #64748B;">Periode Sampai</div>
        <div style="font-weight: bold;">{{ \Carbon\Carbon::parse($dateTo)->format('d F Y') }}</div>
    </div>
    <div style="display: table-cell; width: 33%;">
        <div style="font-size: 10px; color: #64748B;">Dicetak Pada</div>
        <div style="font-weight: bold;">{{ now()->format('d F Y, H:i') }}</div>
    </div>
</div>
...
<div class="content">
    <div style="background: #1565C0; color: white; padding: 12px 16px; border-radius: 8px; margin-bottom: 16px; display: table; width: 100%;">
        <div style="display: table-cell; width: 60%;">
            ...
            <div style="font-size: 20px; font-weight: bold; margin-top: 4px;">Rp {{ number_format($totalAmount, 0, ',', '.') }}</div>
        </div>
        <div style="display: table-cell; width: 40%; text-align: right;">
            ...
            <div style="font-size: 20px; font-weight: bold; margin-top: 4px;">{{ $expenses->count() }}</div>
        </div>
    </div>
    ...
    <table class="cat-table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $cat['name'] }}</td>
                <td class="amount">Rp {{ number_format($cat['total'], 0, ',', '.') }}</td>
                <td class="amount">{{ $totalAmount > 0 ? number_format(($cat['total']/$totalAmount)*100, 1) : 0 }}%</td>
            </tr>
            @endforeach
            <tr class="total-row">
                <td>TOTAL</td>
...
<div class="footer">
    Laporan ini dibuat secara otomatis oleh sistem Budgetly &bull; {{ now()->format('d F Y H:i:s') }} &bull; Budgetly &copy; {{ date('Y') }}
</div>
</body>
</html>
